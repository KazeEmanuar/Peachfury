...
#define j i
    ...
    for (i = 0; i < gMaxSimultaneousNotes * 3; i++) {
        sSampleDmas[gSampleDmaNumListItems].buffer = soundAlloc(&gNotesAndBuffersPool, 1296);
        ...
        sSampleDmas[gSampleDmaNumListItems].bufSize = 1296;
...
out1:
    ...
    for (i = 0; i < gMaxSimultaneousNotes; i++) {
        sSampleDmas[gSampleDmaNumListItems].buffer = soundAlloc(&gNotesAndBuffersPool, 1440);
        if (sSampleDmas[gSampleDmaNumListItems].buffer == NULL) {
#ifdef VERSION_EU
            break;
#else
        ...
        sSampleDmas[gSampleDmaNumListItems].unused2 = 0;
        sSampleDmas[gSampleDmaNumListItems].ttl = 0;
        sSampleDmas[gSampleDmaNumListItems].bufSize = 1440;
        gSampleDmaNumListItems++;
    }
#ifndef VERSION_EU
out2:
...
void preload_sequence(u32 seqId, u8 preloadMask) {
    ...
    if (preloadMask & PRELOAD_SEQUENCE) {
        ...
        if (IS_BANK_LOAD_COMPLETE(seqId) ) {
...
void audio_init() {
    ...
    #define SEQCOUNTMUL4PLUS8 0x200
    gAlBankSets = soundAlloc(&gAudioInitPool, SEQCOUNTMUL4PLUS8);
    audio_dma_copy_immediate((uintptr_t) gBankSetsData, gAlBankSets, SEQCOUNTMUL4PLUS8);
