...
s32 act_dive_picking_up(struct MarioState *m) {
    ...
    stopping_step(m, MARIO_ANIM_STOP_SLIDE_LIGHT_OBJ, ACT_HOLD_IDLE);
    return FALSE;
}
...
s32 act_placing_down(struct MarioState *m) {
    ...
    stopping_step(m, MARIO_ANIM_PLACE_LIGHT_OBJ, ACT_IDLE);
    return FALSE;
}
...
s32 act_throwing(struct MarioState *m) {
    ...
    stopping_step(m, MARIO_ANIM_GROUND_THROW, ACT_IDLE);
    return FALSE;
}
...
s32 act_heavy_throw(struct MarioState *m) {
    ...
    stopping_step(m, MARIO_ANIM_HEAVY_THROW, ACT_IDLE);
    return FALSE;
}
...
s32 act_stomach_slide_stop(struct MarioState *m) {
    ...
    if (m->input & INPUT_ABOVE_SLIDE) {
        return set_mario_action(m, ACT_BEGIN_SLIDING, 0);
    } else if ((m->actionTimer<3) &&(m->input & ((INPUT_A_PRESSED | INPUT_A_LAST_FRAME) | INPUT_B_PRESSED))) {
#ifdef VERSION_SH
    ...
    m->actionTimer++;
    stopping_step(m, MARIO_ANIM_SLOW_LAND_FROM_DIVE, ACT_IDLE);
    return FALSE;
}
...
s32 act_releasing_bowser(struct MarioState *m) {
    ...
    m->angleVel[1] = 0;
    stopping_step(m, MARIO_ANIM_RELEASE_BOWSER, ACT_IDLE);
    return FALSE;
}
