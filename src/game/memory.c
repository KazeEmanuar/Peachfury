...
static void dma_readBackwards(u8 *dest, u8 *srcStart, u8 *srcEnd) {
    u32 size = ALIGN16(srcEnd - srcStart);

    osInvalDCache(dest, size);
    while (size != 0) {
        u32 copySize = (size >= 0x1000) ? 0x1000 : size;
        if ((size<=0x1000+HEADERSIZE) && (size>HEADERSIZE)){
            copySize = size - HEADERSIZE;
        }

        osPiStartDma(&gDmaIoMesg, OS_MESG_PRI_NORMAL, OS_READ, (uintptr_t) (srcStart + size - copySize),
                     (dest + size - copySize), copySize, &gDmaMesgQueue);
        osRecvMesg(&gDmaMesgQueue, &D_80339BEC, OS_MESG_BLOCK);
...
__attribute__((always_inline))  inline struct MarioAnimDmaRelatedThing *func_802789F0(u8 *srcAddr) {
...
s32 load_patchable_table(struct MarioAnimation *a, u32 index) {
    ...
    if (index < sp20->count) {
        ...
        if (a->currentAnimAddr != addr) {
            a->targetAnim->length = 0;
            dma_readBackwards((u8 *) a->targetAnim, addr, addr + size);
