...
#include "print.h"
#include "segment2.h"
//#include "OPT_FOR_SIZE.h" //kazetodo this causes a crash for printing big numbers or something. happens in debugger
 ...
 */
__attribute__((always_inline))  inline void format_integer(s32 n, s32 base, char *dest, s32 *totalLength, u8 width, s8 zeroPad) {
    u32 powBase;
    s32 numDigits = 0;
    s32 i;
    s32 len = 0;
 ...
 * writing to 'zeroPad'.
 */
__attribute__((always_inline))  inline void parse_width_field(const char *str, s32 *srcIndex, u8 *width, s8 *zeroPad) {
...
u8 HDindex[] = { -1, -1 };
u8 HDindexlength = sizeof(HDindex);

u8 isHD(int i) {
    int k;
    for (k = 0; k < HDindexlength; k++) {
        if (i == HDindex[k]) {
            return 1;
        }
    }
 ...
 */
// 53 and 51 are HD
s32 add_glyph_texture(s8 glyphIndex) {
    const u8 *const *glyphs = segmented_to_virtual(main_hud_lut);

    gDPPipeSync(gDisplayListHead++);
    /*if (isHD(glyphIndex)) {
        gDPSetTextureImage(gDisplayListHead++, G_IM_FMT_RGBA, G_IM_SIZ_32b, 1, glyphs[glyphIndex]);
        gSPDisplayList(gDisplayListHead++, dl_hud_img_load_tex_blockHD);
        return 1;
    } else {*/
...
void clip_to_bounds(s32 *x, s32 *y) {
    if (*x < TEXRECT_MIN_X) {
        *x = TEXRECT_MIN_X;
    } else if (*x > TEXRECT_MAX_X) {
        *x = TEXRECT_MAX_X;
    }

    if (*y < TEXRECT_MIN_Y) {
        *y = TEXRECT_MIN_Y;
    } else if (*y > TEXRECT_MAX_Y) {
        *y = TEXRECT_MAX_Y;
    }
}
#endif
...
void render_textrectHD(s32 x, s32 y, s32 pos) {
    s32 rectBaseX = x + pos * 12;
    s32 rectBaseY = 224 - y;
    s32 rectX;
    s32 rectY;
    ...
    rectY = rectBaseY;
    gSPScisTextureRectangle(gDisplayListHead++, rectX << 2, rectY << 2, (rectX + 15) << 2,
                            (rectY + 15) << 2, G_TX_RENDERTILE, 0, 0, 4 << 10, 1 << 11);
}
/**
    ...
    for (i = 0; i < sTextLabelsCount; i++) {
        for (j = 0; j < sTextLabels[i]->length; j++) {
            ...
            if (glyphIndex != GLYPH_SPACE) {
                HD = add_glyph_texture(glyphIndex);
                /*if (HD) {
                    render_textrectHD(sTextLabels[i]->x, sTextLabels[i]->y, j);
                } else {
*/
