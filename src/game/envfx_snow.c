...
s32 envfx_init_snow(s32 mode) {
    ...
    switch (mode) {
        ...
        case ENVFX_MODE_RAIN:
            if (isEmu) {
                gSnowParticleMaxCount = RAINCOUNT * 2;
                gSnowParticleCount = RAINCOUNT * 2;
            } else {
                gSnowParticleMaxCount = RAINCOUNT;
                gSnowParticleCount = RAINCOUNT;
            }
            break;
    }
    ...
    for (i = 0; i < gSnowParticleMaxCount; i++) {
        (gEnvFxBuffer + i)->animFrame = 256;
    }
    gEnvFxMode = mode;
    return 1;
}
...
s32 envfx_is_snowflake_alive(s32 index, s32 snowCylinderX, s32 snowCylinderY, s32 snowCylinderZ) {
    ...
    if (sqr(x - snowCylinderX) + sqr(z - snowCylinderZ) > sqr(320)) {
        return 0;
    }

    if ((y < snowCylinderY - 240) || (snowCylinderY + 240 < y)) {
        return 0;
    }
...
void envfx_update_snow_normal(s32 snowMode) {
    s32 i;
    s32 mathSins;
    s16 randomAngle;
    if (snowMode == ENVFX_MODE_CYANBUBBLE) {
        for (i = 0; i < gSnowParticleCount; i++) {
           // (gEnvFxBuffer + i)->isAlive = (gEnvFxBuffer + i)->animFrame < 255;
            if (!((gEnvFxBuffer + i)->animFrame < 255)) {
                randomAngle = random_u16();
#define SPAWNRANGE 3800.f
                (gEnvFxBuffer + i)->xPos = sins(randomAngle) * (SPAWNRANGE * random_float());
                (gEnvFxBuffer + i)->zPos = coss(randomAngle) * (SPAWNRANGE * random_float()) * 1.4f;
                (gEnvFxBuffer + i)->yPos = -1350.f;
              //  (gEnvFxBuffer + i)->isAlive = 1;
                (gEnvFxBuffer + i)->animFrame = 0;
            } else {
                mathSins = (gEnvFxBuffer + i)->xPos * 0x10 + (gEnvFxBuffer + i)->yPos * 0x80
                           + (gEnvFxBuffer + i)->zPos * 0x10;
                (gEnvFxBuffer + i)->xPos += 2.f * sins(mathSins);
                (gEnvFxBuffer + i)->yPos += 5 + (gEnvFxBuffer + i)->animFrame / 32;
                (gEnvFxBuffer + i)->zPos += 2.f * coss(mathSins);
        ...
        for (i = 0; i < gSnowParticleCount; i++) {
            ...
            if (!((gEnvFxBuffer + i)->animFrame < 255)) {
                randomAngle = random_u16();
#define SPAWNRANGE 2900.f
#define SPAWNOFFSET 1600.f
                (gEnvFxBuffer + i)->xPos =
                    sins(randomAngle) * (SPAWNRANGE * random_float() + SPAWNOFFSET);
                (gEnvFxBuffer + i)->zPos =
                    coss(randomAngle) * (SPAWNRANGE * random_float() + SPAWNOFFSET);
                (gEnvFxBuffer + i)->yPos = gMarioState->pos[1] - 800 - 1000.0f * random_float();
                //(gEnvFxBuffer + i)->isAlive = 1;
                (gEnvFxBuffer + i)->animFrame = 0;
            } else {
                mathSins = (gEnvFxBuffer + i)->xPos * 0x10 + (gEnvFxBuffer + i)->yPos * 0x80
...
void envfx_update_snow_blizzard(s32 snowCylinderX, s32 snowCylinderY, s32 snowCylinderZ) {
    ...
    for (i = 0; i < gSnowParticleCount; i++) {
        ...
        if (!envfx_is_snowflake_alive(i, snowCylinderX, snowCylinderY, snowCylinderZ)) {
            (gEnvFxBuffer + i)->xPos =
...
extern void *mat_lava_f3d_material;
extern void *RHOMBUS;
void append_flame_vertex_buffer(Gfx *gfx, s32 index, Vec3s vertex1, Vec3s vertex2, Vec3s vertex3,
                                s32 snowMode) {
    s32 i = 0;
    ...
    for (i = 0; i < 30; i += 3) { // FF8B22 maxcolor, 221D19 mincolor
        if (snowMode == ENVFX_MODE_CYANBUBBLE) {
            col[0] = 127 /* - gEnvFxBuffer[index + i / 3].animFrame*/;
            col[1] = 255 /*- gEnvFxBuffer[index + i / 3].animFrame/2*/;
            col[2] = 255 /* - gEnvFxBuffer[index + i / 3].animFrame/4*/;
        } else {
            col[0] = 255 /* - gEnvFxBuffer[index + i / 3].animFrame*/;
            col[1] = 128 /*- gEnvFxBuffer[index + i / 3].animFrame/2*/;
            col[2] = 65 /* - gEnvFxBuffer[index + i / 3].animFrame/4*/;
        }
        tcs = 1024 + (gEnvFxBuffer[index + i / 3].animFrame) * 8;
        ...
        vertBuf[i].v.ob[1] = gEnvFxBuffer[index + i / 3].yPos + vertex1[1];
        vertBuf[i].v.ob[2] = gEnvFxBuffer[index + i / 3].zPos + vertex1[2];
        vertBuf[i].v.cn[0] = col[0];
        vertBuf[i].v.cn[1] = col[1];
        vertBuf[i].v.cn[2] = col[2];

        vertBuf[i+1].v.tc[0] = 0;
        vertBuf[i+1].v.tc[1] = 960;
        vertBuf[i + 1].v.ob[0] = gEnvFxBuffer[index + i / 3].xPos + vertex2[0];
        vertBuf[i + 1].v.ob[1] = gEnvFxBuffer[index + i / 3].yPos + vertex2[1];
        vertBuf[i + 1].v.ob[2] = gEnvFxBuffer[index + i / 3].zPos + vertex2[2];
        vertBuf[i + 1].v.cn[0] = col[0];
        vertBuf[i + 1].v.cn[1] = col[1];
        vertBuf[i + 1].v.cn[2] = col[2];
        vertBuf[i + 1].v.tc[1] = tcs;

        vertBuf[i+2].v.tc[0] = 960;
        vertBuf[i+2].v.tc[1] = 0;
        vertBuf[i + 2].v.ob[0] = gEnvFxBuffer[index + i / 3].xPos + vertex3[0];
        vertBuf[i + 2].v.ob[1] = gEnvFxBuffer[index + i / 3].yPos + vertex3[1];
        vertBuf[i + 2].v.ob[2] = gEnvFxBuffer[index + i / 3].zPos + vertex3[2];
        vertBuf[i + 2].v.cn[0] = col[0];
        vertBuf[i + 2].v.cn[1] = col[1];
        vertBuf[i + 2].v.cn[2] = col[2];
        vertBuf[i + 2].v.tc[0] = tcs;
    }

    gSPVertex(gfx, VIRTUAL_TO_PHYSICAL(vertBuf), 15, 0);
}
struct SnowFlakeVertex gFlameFlakeVertex1 = { -50, 50, 0 };
struct SnowFlakeVertex gFlameFlakeVertex2 = { -50, -50, 0 };
struct SnowFlakeVertex gFlameFlakeVertex3 = { 50, 50, 0 };
Gfx *envfx_update_flames(s32 snowMode, Vec3s marioPos, Vec3s camFrom, Vec3s camTo) {
    s32 i;
    ...
    vertex1 = gFlameFlakeVertex1;
    vertex2 = gFlameFlakeVertex2;
    vertex3 = gFlameFlakeVertex3;
    ...
    envfx_update_snow_normal(snowMode);

    rotate_triangle_vertices((s16 *) &vertex1, (s16 *) &vertex2, (s16 *) &vertex3, pitch, yaw);
    if (snowMode == ENVFX_MODE_CYANBUBBLE) {
        gSPDisplayList(gfx++, &RHOMBUS); // snowflake with gray edge
    } else {
        gSPDisplayList(gfx++, &mat_lava_f3d_material); // snowflake with gray edge
    }

    for (i = 0; i < gSnowParticleCount; i += 10) {
        append_flame_vertex_buffer(gfx++, i, (s16 *) &vertex1, (s16 *) &vertex2, (s16 *) &vertex3,
                                   snowMode);
        ...
        // append 30 at once to buffer
    }

    gSPBranchList(gfx++, &tiny_bubble_dl_0B006AB0);
...
void append_rain_vertex_buffer(Gfx *gfx, s32 index, Vec3s vertex1, Vec3s vertex2, Vec3s vertex3,
                               Vec3s vertex4) {
    s32 i = 0;
    Vtx *vertBuf = (Vtx *) alloc_display_list(32 * sizeof(Vtx));

    for (i = 0; i < 8; i++) {
        vertBuf[i * 4] = gSnowTempVtx[0];
        vertBuf[i * 4].v.ob[0] = gEnvFxBuffer[index + i].xPos + vertex1[0];
        vertBuf[i * 4].v.ob[1] = gEnvFxBuffer[index + i].yPos + vertex1[1];
        vertBuf[i * 4].v.ob[2] = gEnvFxBuffer[index + i].zPos + vertex1[2];
        vertBuf[i * 4 + 1] = gSnowTempVtx[0];
        vertBuf[i * 4 + 1].v.ob[0] = gEnvFxBuffer[index + i].xPos + vertex2[0];
        vertBuf[i * 4 + 1].v.ob[1] = gEnvFxBuffer[index + i].yPos + vertex2[1];
        vertBuf[i * 4 + 1].v.ob[2] = gEnvFxBuffer[index + i].zPos + vertex2[2];
        vertBuf[i * 4 + 2] = gSnowTempVtx[0];
        vertBuf[i * 4 + 2].v.ob[0] = gEnvFxBuffer[index + i].xPos + vertex3[0];
        vertBuf[i * 4 + 2].v.ob[1] = gEnvFxBuffer[index + i].yPos + vertex3[1];
        vertBuf[i * 4 + 2].v.ob[2] = gEnvFxBuffer[index + i].zPos + vertex3[2];
        vertBuf[i * 4 + 3] = gSnowTempVtx[0];
        vertBuf[i * 4 + 3].v.ob[0] = gEnvFxBuffer[index + i].xPos + vertex4[0];
        vertBuf[i * 4 + 3].v.ob[1] = gEnvFxBuffer[index + i].yPos + vertex4[1];
        vertBuf[i * 4 + 3].v.ob[2] = gEnvFxBuffer[index + i].zPos + vertex4[2];
    }
    gSPVertex(gfx, VIRTUAL_TO_PHYSICAL(vertBuf), 32, 0);
}

inline void rotate_quad_vertices(Vec3s vertex1, Vec3s vertex2, Vec3s vertex3, Vec3s vertex4, s16 pitch,
                          s16 yaw) {
    f32 cosPitch = coss(pitch);
    f32 sinPitch = sins(pitch);
    f32 cosMYaw = coss(-yaw);
    ...
    vertex1[0] = v1[0] * cosMYaw + v1[1] * (sinPitch * sinMYaw);
    vertex1[1] = v1[1] * cosPitch;
    vertex1[2] = v1[0] * sinMYaw + v1[1] * (-sinPitch * cosMYaw);

    vertex2[0] = v2[0] * cosMYaw + v2[1] * (sinPitch * sinMYaw);
    vertex2[1] = v2[1] * cosPitch;
    vertex2[2] = v2[0] * sinMYaw + v2[1] * (-sinPitch * cosMYaw);

    vertex3[0] = v3[1] * (sinPitch * sinMYaw);
    vertex3[1] = v3[1] * cosPitch;
    vertex3[2] = v3[1] * (-sinPitch * cosMYaw) ;

    vertex4[0] = v4[1] * (sinPitch * sinMYaw);
    vertex4[1] = v4[1] * cosPitch ;
    vertex4[2] = v4[1] * (-sinPitch * cosMYaw);
}
u8 rainCount = RAINCOUNT;

extern const Gfx rain_material[];
#define RAINSIZE 104
#define RAINWIDTH 2
struct SnowFlakeVertex gRainFlakeVertex1 = { -RAINWIDTH, RAINSIZE, 0 };
struct SnowFlakeVertex gRainFlakeVertex2 = { -RAINWIDTH, -RAINSIZE, 0 };
struct SnowFlakeVertex gRainFlakeVertex3 = { 0, RAINSIZE, 0 };
struct SnowFlakeVertex gRainFlakeVertex4 = { 0, -RAINSIZE, 0 };
#include "game/level_update.h"
Gfx *envfx_update_rain(s32 snowMode, Vec3s marioPos, Vec3s camFrom, Vec3s camTo) {
    s32 i;
    s16 radius, pitch, yaw;
    Vec3s snowCylinderPos;
    struct SnowFlakeVertex vertex1, vertex2, vertex3, vertex4;
    Gfx *gfxStart;
    Gfx *gfx;
    s16 dir;
    if (isEmu) {
    ...
    gfxStart = (Gfx *) alloc_display_list(((rainCount / 8) * 9 + 5) * sizeof(Gfx));
    gfx = gfxStart;

#define SPAWNRADIUS 6000.f
#define SPAWNHEIGHT 6800.f
    orbit_from_positions(camTo, camFrom, &radius, &pitch, &yaw);
    // yaw = atan2s(camFrom[2] - camTo[2], camFrom[0] - camTo[0]);
    if (!(gEnvFxBuffer + i)->isAlive) {
        for (i = 0; i < rainCount; i++) {
            (gEnvFxBuffer + i)->xPos =
                random_float() * SPAWNRADIUS - SPAWNRADIUS / 2.f + gLakituState.focus[0];
            (gEnvFxBuffer + i)->zPos =
                random_float() * SPAWNRADIUS - SPAWNRADIUS / 2.f + gLakituState.focus[2];
            (gEnvFxBuffer + i)->yPos = 132.f * i - 1200.f;
            (gEnvFxBuffer + i)->isAlive = 1;
        }
    }
    for (i = 0; i < rainCount; i++) {
        if ((gEnvFxBuffer + i)->yPos < -1400.f) {
#define SPAWNRANGE 0x8000
            dir = (random_u16() % SPAWNRANGE) - SPAWNRANGE / 2;
            (gEnvFxBuffer + i)->xPos =
                random_float() * SPAWNRADIUS * sins(yaw + dir + 0x8000) + gLakituState.focus[0];
            (gEnvFxBuffer + i)->zPos =
                random_float() * SPAWNRADIUS * coss(yaw + dir + 0x8000) + gLakituState.focus[2];
            (gEnvFxBuffer + i)->yPos = SPAWNHEIGHT;

        } else {
            (gEnvFxBuffer + i)->yPos -= 150;
            (gEnvFxBuffer + i)->xPos += 15;
        }
    }

    vertex1 = gRainFlakeVertex1;
    vertex2 = gRainFlakeVertex2;
    vertex3 = gRainFlakeVertex3;
    vertex4 = gRainFlakeVertex4;
    rotate_quad_vertices((s16 *) &vertex1, (s16 *) &vertex2, (s16 *) &vertex3, (s16 *) &vertex4,
                         pitch / 2, yaw);

    gSPDisplayList(gfx++, &rain_material);
    gDPSetPrimColor(gfx++, 0, 0, 150, 150, 200, 255);

    for (i = 0; i < rainCount; i += 8) {
        append_rain_vertex_buffer(gfx++, i, (s16 *) &vertex1, (s16 *) &vertex2, (s16 *) &vertex3,
                                  (s16 *) &vertex4);
...
Gfx *envfx_update_particles(s32 mode, Vec3s marioPos, Vec3s camTo, Vec3s camFrom) {
    ...
    switch (mode) {
        ...
        case ENVFX_SNOW_NORMAL:
        case ENVFX_MODE_CYANBUBBLE:
            gfx = envfx_update_flames(mode, marioPos, camFrom, camTo);
            break;
        ...
        case ENVFX_SNOW_BLIZZARD:
            ...
            break;
        case ENVFX_MODE_RAIN:
            gfx = envfx_update_rain(3, marioPos, camFrom, camTo);
            break;
