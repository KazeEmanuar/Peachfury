...
s32 is_anim_at_end(struct MarioState *m) {
    ...
    if ((!o->header.gfx.unk38.curAnim) || o->header.gfx.unk38.animFrame == 0) {
        return 0;
    }
...
s16 set_mario_anim_with_accel(struct MarioState *m, s32 targetAnimID, s32 accel) {
...
#ifdef ANIMSWAP
    if (!m->animationSwapID) {
        m->animIdSwap = targetAnimID;
        targetAnim = m->animationSwap->targetAnim;
        if (load_patchable_table(m->animationSwap, targetAnimID)) {
            if (!m->animSwappedThisFrame) {
                m->animationSwapID = 1;
            }
        }
    } else {
#endif
        m->animIDReal = targetAnimID;
        targetAnim = m->animation->targetAnim;
        if (load_patchable_table(m->animation, targetAnimID)) {
#ifdef ANIMSWAP
            if (!m->animSwappedThisFrame) {
                m->animationSwapID = 0;
    ...
    if (o->header.gfx.unk38.animID != targetAnimID) {
        ...
        if (targetAnimID == MARIO_ANIM_LAND_FROM_SINGLE_JUMP) {
            o->header.gfx.unk38.animFrame = 19;
        } else if (targetAnimID == MARIO_ANIM_GENERAL_LAND) {
            o->header.gfx.unk38.animFrame = 6;
        } else {
...
s32 is_anim_past_frame(struct MarioState *m, s16 animFrame) {
    ...
    struct GraphNodeObject_sub *animInfo = &m->marioObj->header.gfx.unk38;
    struct Animation *curAnim = animInfo->curAnim;
    if ((!curAnim) || (m->marioObj->header.gfx.unk38.animFrame == 0)) {
        return 0;
    }
    if (animInfo->animAccel) {
        /* if (curAnim->flags & ANIM_FLAG_FORWARD) {
...
void adjust_sound_for_speed(struct MarioState *m) {
    ...
    adjustBankPitch(1, (absForwardVel > 100) ? 100 : absForwardVel);
}

s16 terrainSounds[6][16] = {
    { SOUND_ACTION_TERRAIN_JUMP >> 16, (SOUND_ACTION_TERRAIN_JUMP + (1 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_JUMP + (2 << 16)) >> 16, (SOUND_ACTION_TERRAIN_JUMP + (3 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_JUMP + (4 << 16)) >> 16, (SOUND_ACTION_TERRAIN_JUMP + (5 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_JUMP + (6 << 16)) >> 16, (SOUND_ACTION_TERRAIN_JUMP + (7 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STUCK_IN_GROUND + (1 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STUCK_IN_GROUND + (4 << 16)) >> 16, (SOUND_ACTION_TERRAIN_JUMP) >> 16,
      (SOUND_ACTION_TERRAIN_JUMP) >> 16, (SOUND_ACTION_TERRAIN_JUMP) >> 16,
      (SOUND_ACTION_TERRAIN_JUMP) >> 16, (SOUND_ACTION_TERRAIN_JUMP) >> 16,
      (SOUND_ACTION_TERRAIN_JUMP) >> 16 },

    { SOUND_ACTION_TERRAIN_LANDING >> 16, (SOUND_ACTION_TERRAIN_LANDING + (1 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING + (2 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING + (3 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING + (4 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING + (5 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING + (6 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING + (7 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STUCK_IN_GROUND + (2 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STUCK_IN_GROUND + (5 << 16)) >> 16, (SOUND_ACTION_TERRAIN_LANDING) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING) >> 16, (SOUND_ACTION_TERRAIN_LANDING) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING) >> 16, (SOUND_ACTION_TERRAIN_LANDING) >> 16,
      (SOUND_ACTION_TERRAIN_LANDING) >> 16 },

    { SOUND_ACTION_TERRAIN_STEP >> 16, (SOUND_ACTION_TERRAIN_STEP + (1 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP + (2 << 16)) >> 16, (SOUND_ACTION_TERRAIN_STEP + (3 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP + (4 << 16)) >> 16, (SOUND_ACTION_TERRAIN_STEP + (5 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP + (6 << 16)) >> 16, (SOUND_ACTION_TERRAIN_STEP + (7 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STUCK_IN_GROUND + (3 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STUCK_IN_GROUND + (6 << 16)) >> 16, (SOUND_ACTION_TERRAIN_STEP) >> 16,
      (SOUND_ACTION_TERRAIN_STEP) >> 16, (SOUND_ACTION_TERRAIN_STEP) >> 16,
      (SOUND_ACTION_TERRAIN_STEP) >> 16, (SOUND_ACTION_TERRAIN_STEP) >> 16,
      (SOUND_ACTION_TERRAIN_STEP) >> 16 },

    { SOUND_ACTION_TERRAIN_BODY_HIT_GROUND >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND + (1 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND + (2 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND + (3 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND + (4 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND + (5 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND + (6 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND + (7 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND + (3 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND) >> 16, (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND) >> 16, (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND) >> 16, (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND) >> 16,
      (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND) >> 16 },

    { SOUND_ACTION_TERRAIN_STEP_TIPTOE >> 16, (SOUND_ACTION_TERRAIN_STEP_TIPTOE + (1 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE + (2 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE + (3 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE + (4 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE + (5 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE + (6 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE + (7 << 16)) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE + (3 << 16)) >> 16, (SOUND_ACTION_TERRAIN_STEP_TIPTOE) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE) >> 16, (SOUND_ACTION_TERRAIN_STEP_TIPTOE) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE) >> 16, (SOUND_ACTION_TERRAIN_STEP_TIPTOE) >> 16,
      (SOUND_ACTION_TERRAIN_STEP_TIPTOE) >> 16, (SOUND_ACTION_TERRAIN_STEP_TIPTOE) >> 16 },

    { SOUND_MOVING_TERRAIN_SLIDE >> 16, (SOUND_MOVING_TERRAIN_SLIDE + (1 << 16)) >> 16,
      (SOUND_MOVING_TERRAIN_SLIDE + (2 << 16)) >> 16, (SOUND_MOVING_TERRAIN_SLIDE + (3 << 16)) >> 16,
      (SOUND_MOVING_TERRAIN_SLIDE + (4 << 16)) >> 16, (SOUND_MOVING_TERRAIN_SLIDE + (5 << 16)) >> 16,
      (SOUND_MOVING_TERRAIN_SLIDE + (6 << 16)) >> 16, (SOUND_MOVING_TERRAIN_SLIDE + (7 << 16)) >> 16,
      (SOUND_MOVING_TERRAIN_SLIDE) >> 16, (SOUND_MOVING_TERRAIN_SLIDE) >> 16,
      (SOUND_MOVING_TERRAIN_SLIDE) >> 16, (SOUND_MOVING_TERRAIN_SLIDE) >> 16,
      (SOUND_MOVING_TERRAIN_SLIDE) >> 16, (SOUND_MOVING_TERRAIN_SLIDE) >> 16,
      (SOUND_MOVING_TERRAIN_SLIDE) >> 16, (SOUND_MOVING_TERRAIN_SLIDE) >> 16 }
};
 ...
 */
// not optimized for editability
void play_sound_and_spawn_particles(struct MarioState *m, u32 soundBits, u32 waveParticleType) {
    switch (m->terrainParticleID) {
        case PARTICLE_TERRAIN_SAND:
            m->particleFlags |= PARTICLE_DIRT;
            break;
        case PARTICLE_TERRAIN_SNOW:
            m->particleFlags |= PARTICLE_SNOW;
            break;
        case PARTICLE_TERRAIN_LEAFS:
            m->particleFlags |= PARTICLE_LEAF;
            break;
        case PARTICLE_TERRAIN_WATER:
            ...
            break;
        case PARTICLE_TERRAIN_SPARKLES:
            m->particleFlags |= PARTICLE_SPARKLES;
            break;
    }

    if (soundBits == SOUND_ACTION_UNSTUCK_FROM_GROUND || soundBits == SOUND_MARIO_PUNCH_HOO) {
        play_sound(soundBits, m->marioObj->header.gfx.cameraToObject);
    } else {
        if ((soundBits >> 16) == (SOUND_ACTION_TERRAIN_JUMP >> 16)) {
            play_sound((terrainSounds[0][m->terrainSoundAddend >> 16] << 16) | (soundBits & 0xFFFF),
                       m->marioObj->header.gfx.cameraToObject);
        } else if ((soundBits >> 16) == (SOUND_ACTION_TERRAIN_LANDING >> 16)) {
            play_sound((terrainSounds[1][m->terrainSoundAddend >> 16] << 16) | (soundBits & 0xFFFF),
                       m->marioObj->header.gfx.cameraToObject);
        } else if ((soundBits >> 16) == (SOUND_ACTION_TERRAIN_STEP >> 16)) {
            play_sound((terrainSounds[2][m->terrainSoundAddend >> 16] << 16) | (soundBits & 0xFFFF),
                       m->marioObj->header.gfx.cameraToObject);
        } else if ((soundBits >> 16) == (SOUND_ACTION_TERRAIN_BODY_HIT_GROUND >> 16)) {
            play_sound((terrainSounds[3][m->terrainSoundAddend >> 16] << 16) | (soundBits & 0xFFFF),
                       m->marioObj->header.gfx.cameraToObject);
        } else if ((soundBits >> 16) == (SOUND_ACTION_TERRAIN_STEP_TIPTOE >> 16)) {
            play_sound((terrainSounds[4][m->terrainSoundAddend >> 16] << 16) | (soundBits & 0xFFFF),
                       m->marioObj->header.gfx.cameraToObject);
        } else if ((soundBits >> 16) == (SOUND_MOVING_TERRAIN_SLIDE >> 16)) {
            play_sound((terrainSounds[5][m->terrainSoundAddend >> 16] << 16) | (soundBits & 0xFFFF),
                       m->marioObj->header.gfx.cameraToObject);
 ...
 */
inline void play_mario_landing_sound(struct MarioState *m, u32 soundBits) {
 ...
 */
inline void play_mario_landing_sound_once(struct MarioState *m, u32 soundBits) {
 ...
 */
s32 mario_get_floor_class(struct Surface *floor) {
    s32 floorClass;
    if (floor) {
        /*print_text_fmt_int(85, 10, "%02x", floor->type & 0xFF);
        print_text_fmt_int(60, 10, "%02x", (floor->type & 0x0000FF00) >> 8);
        print_text_fmt_int(35, 10, "%02x", (floor->type & 0x00FF0000) >> 16);
        print_text_fmt_int(10, 10, "%02x", (floor->type & 0xFF000000) >> 24);*/
        if (floor->type & (SPECFLAG_SLIPPERY << 8)) {
            return SURFACE_CLASS_SLIPPERY;
        } else if (floor->type & (SPECFLAG_CLIMBABLE << 8)) {
            return SURFACE_CLASS_NOT_SLIPPERY;
        } else if (floor->type & (SPECFLAG_MEGASLIP << 8)) {
    ...
    if (gMarioState->action == ACT_CRAWLING && gMarioState->floor->normal.y > 0.5f
        && floorClass == SURFACE_CLASS_DEFAULT) {
...
u32 mario_get_terrain_sound_addend(struct MarioState *m) {
    ...
    if (m->floor) {
        ...
        if (m->floorHeight < (m->waterLevel - 10)) {
            ...
            ret = (m->floor->type & 0xF0000000) >> 12;
        }
    }
    m->terrainParticleID = (m->floor->type & 0x0F000000) >> 24;
    return ret;
}
 ...
 */
struct Surface *resolve_and_return_wall_collisions(Vec3f pos, f32 offset,
                                                   f32 radius) { // changed for wallcuck
    struct WallCollisionData collisionData;
    struct Surface *wall = NULL;
    int i;
    u16 anglediff;
    u16 anglediffstore = 0x8000;
...
static u32 set_mario_action_moving(struct MarioState *m, u32 action, UNUSED u32 actionArg) {
    s16 floorClass = mario_get_floor_class(m->floor);
...
u8 mario_can_get_up(struct MarioState *m) { // crawlspaces
    return (m->ceilHeight - 160.f > m->pos[1]);
}
...
void squish_mario_model(struct MarioState *m) {
    f32 y;
    f32 diff = m->vel[1] - m->prevYvel;
    // f32 deform = m->marioObj->header.gfx.scale[1];
    if (m->pos[1] == m->floorHeight) {
        diff = -diff;
    }
    if ((m->vel[1] == m->prevYvel) & (m->vel[1] == -75.f)) {
        diff = 4.f;
    }
    y = m->marioObj->header.gfx.scale[1];
    if (m->squishTimer != 0xFF) {
        if (m->squishTimer == 0) {
            m->squishSpeed += diff;
            m->squishSpeed -= (m->squishSpeed * 0.05f);
            m->squishSpeed = approach_f32_symmetric(m->squishSpeed, 0.f, 1.f);
            m->squishSpeed *= .99f;
            m->squishSpeed -= m->squishyness * 0.1f;
            m->squishyness += m->squishSpeed;
            // y = 1.f + m->squishyness/75.f;
            y = approach_f32_asymptotic(y, m->squishyness / 800.f + 1.f, 0.05f);
        } else if (m->squishTimer <= 16) {
            m->squishSpeed = 0;
            m->squishTimer -= 1;
            y = 1.0f - ((sSquishScaleOverTime[15 - m->squishTimer] * 0.6f) / 100.0f);
        } else {
            m->squishSpeed = 0;
            m->squishTimer -= 1;
            y = .4f;
    ...
    m->prevYvel = m->vel[1];
    vec3f_set(m->marioObj->header.gfx.scale, sqrtf(1.f / y), y, sqrtf(1.f / y));
}
...
void update_mario_joystick_inputs(struct MarioState *m) {
    ...
    if (m->intendedMag > 0.0f) {
        m->intendedYaw = atan2s(-controller->stickY, controller->stickX) + newcam_yaw;
...
void update_mario_geometry_inputs(struct MarioState *m) {
    ...
    if (((m->action & ACT_FLAG_RIDING_SHELL) || (m->flags & MARIO_VANISH_CAP))
        ...
        m->floorHeight = m->waterLevel + ICEFLOWERWALKOFFSTE;
        m->floor = &gWaterSurfacePseudoFloor;
        m->floor->originOffset = m->waterLevel + ICEFLOWERWALKOFFSTE;
    }
    if (m->floor) {
...
void update_mario_inputs(struct MarioState *m) {
    ...
    m->flags &= 0xFFFFFF;
    if (gCurrentArea->camera->cutscene != 0xc9) {
        /*update_mario_button_inputs(m);
        if (lastFrameA) {
            m->input |= INPUT_A_LAST_FRAME;
        }
            ...
            if (!currentScene) {
                if ((m->action & ACT_FLAG_SWIMMING) && ((m->action & ACT_FLAG_INTANGIBLE) == 0)) {
                    terrainIsSnow =
                        ...
                        || (m->floor && (SURFACETYPE(m->floor) == SURFACE_PAINTING_WARP_F8));
#define BREATHSCALE 7000.f
                    if (m->pos[1] < (m->waterLevel - 140)) {
                        breath = approach_s16_symmetric(
                            breath, 0,
                            (terrainIsSnow ? 3 : 1)
                                * (1 + ((u16) ((m->waterLevel - 140 - m->pos[1]) / BREATHSCALE))));
                        if (breath < BREATHMETERBAR / 2) {
                        ...
                        breath = approach_s16_symmetric(breath, MAXBREATH, 0x1A);
                    }
                } else {
                    breath = approach_s16_symmetric(breath, MAXBREATH, 0x1A);
                }
    ...
    if (!fadeoutstarted) {
        ...
        if (fadeoutstarted == 18) {
            ...
            m->vel[1] = 0;
            m->forwardVel = 0;
            m->floorHeight = find_floor_marioair(m->pos[0], m->pos[1], m->pos[2], &m->floor);
...
s32 execute_mario_action(UNUSED struct Object *o) {
    s32 inLoop = TRUE;
    u32 lastFrameA = gMarioState->input & INPUT_A_PRESSED;
    ...
    if (gMarioState->action) {
        ...
        if (gCurrentArea->camera->cutscene != 0xc9) {
            update_mario_button_inputs(gMarioState);
            if (lastFrameA) {
                gMarioState->input |= INPUT_A_LAST_FRAME;
            }
            update_mario_joystick_inputs(gMarioState);
            if ((gMarioState->controller->buttonPressed & L_TRIG)
                && (gMarioState->action & ACT_FLAG_IDLE)) { // newcam
            ...
            if (!(gMarioState->input & (INPUT_NONZERO_ANALOG | INPUT_A_PRESSED))) {
                gMarioState->input |= INPUT_UNKNOWN_5;
            }
        }
        if (gMarioState->flags & MARIO_VANISH_CAP) {
            if (gMarioState->floorHeight < gMarioState->waterLevel) {
                gMarioState->floorHeight = gMarioState->waterLevel + ICEFLOWERWALKOFFSTE;
                gMarioState->floor = &gWaterSurfacePseudoFloor;
                gMarioState->floor->originOffset =
                    gMarioState->waterLevel + ICEFLOWERWALKOFFSTE; //! Negative origin offset
            }
        }
        ...
        o->header.gfx.pos[1] += gMarioState->numKeys * 2;
        // Both of the wind handling portions play wind audio only in
        // non-Japanese releases.
        if (SURFACETYPE(gMarioState->floor) == SURFACE_HORIZONTAL_WIND) {
            spawn_wind_particles(0, (gMarioState->floor->force << 8));
            play_sound(SOUND_ENV_WIND2, gMarioState->marioObj->header.gfx.cameraToObject);
        }

        if (SURFACETYPE(gMarioState->floor) == SURFACE_VERTICAL_WIND) {
...
void init_mario(void) {
    ...
    if (stated && (gCurrCourseNum == 0)) {
        capObject = spawn_object(gMarioObject, 0x6a, bhvStateYosh);
        capObject->oPosX += sins(gMarioState->faceAngle[1]) * 3500.f;
        capObject->oPosZ += coss(gMarioState->faceAngle[1]) * 3500.f;
        capObject->oPosY += 8000.f;
        stated = 0;
    }
}
...
void init_mario_from_save_file(void) {
    ...
    gMarioState->animation = &gmAnim;
#ifdef ANIMSWAP
    gMarioState->animationSwap = &gmAnimSwap;
#endif
    gMarioState->animationSwapID = 0;
    gMarioState->prevYvel = 0;
    gMarioState->squishyness = 0;
