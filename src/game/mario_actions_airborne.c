...
#pragma GCC diagnostic pop
    ...
    if (m->action != ACT_TWIRLING && !(m->floor->type & (SPECFLAG_BURNING << 8))) {
        if (m->vel[1] < -55.0f) {
...
s32 should_get_stuck_in_ground(struct MarioState *m) {
    ...
    if (floor && (floor->type & (SPECFLAG_CANGETSUCK<<8))) {
...
s32 check_horizontal_wind(struct MarioState *m) {
    ...
    if (SURFACETYPE(floor) == SURFACE_HORIZONTAL_WIND) {
...
__attribute__((always_inline))  inline void update_flying_yaw(struct MarioState *m) {
...
__attribute__((always_inline))  inline void update_flying_pitch(struct MarioState *m) {
...
__attribute__((always_inline))  inline void update_flying(struct MarioState *m) {
...
s32 act_freefall(struct MarioState *m) {
    ...
    if (TimeSinceLeftFloor < EXTRATIME) {
        if (landTimeLeft) {
            landTimeLeft--;
        }
        if (m->input & (INPUT_A_PRESSED | INPUT_A_LAST_FRAME)) {
            if (landTimeLeft || (m->prevAction == ACT_STOMACH_SLIDE)
                || (m->prevAction == ACT_DIVE_SLIDE)) {
                switch (m->prevAction) {
    ...
    common_air_action_step(m, ACT_FREEFALL_LAND, animation,
                           AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG);
    return FALSE;
}
...
s32 act_hold_freefall(struct MarioState *m) {
    ...
    common_air_action_step(m, ACT_HOLD_FREEFALL_LAND, animation,
                           AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG);
    return FALSE;
}
...
s32 act_long_jump(struct MarioState *m) {
    ...
    if (SURFACETYPE(m->floor) == SURFACE_VERTICAL_WIND && m->actionState == 0) {
        play_sound(SOUND_MARIO_HERE_WE_GO, m->marioObj->header.gfx.cameraToObject);
        m->actionState = 1;
    }

    common_air_action_step(m, ACT_LONG_JUMP_LAND, animation,
                           AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG);
#ifdef VERSION_SH
...
s32 act_ground_pound(struct MarioState *m) {
    ...
    if (gMarioState->controller->buttonPressed & B_BUTTON) {
        ...
        gMarioState->vel[1] = 24.f;
        gMarioState->faceAngle[1] =
            (m->intendedMag / 32.f) * (s16) (m->intendedYaw - m->faceAngle[1]) + m->faceAngle[1];
        play_sound(SOUND_OBJ_UNKNOWN4, gDefaultSoundArgs);
    }
    return FALSE;
}
...
s32 check_wall_kick(struct MarioState *m) {
    if ((m->input & (INPUT_A_PRESSED | INPUT_A_LAST_FRAME)) && m->wallKickTimer != 0 && m->prevAction == ACT_AIR_HIT_WALL) {
        m->faceAngle[1] += 0x8000;
...
s32 act_wall_slide(struct MarioState *m) {
    ...
    m->forwardVel = approach_f32_symmetric(m->forwardVel, 24.f, 2.f);
    if (check_wall_kick(m)) {
        m->faceAngle[1] -= 0x8000;
        return 1;
    }
    ...
    m->pos[0] -= m->wall->normal.x * 2.f /* * 2.f*/;
    m->pos[2] -= m->wall->normal.z * 2.f /* * 2.f*/;
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:
...
#define ELEVATORSPEED 8.8f
    if (m->wall) {
        if (SURFACETYPE(m->wall) == SURFACE_PAINTING_WOBBLE_AE) {
            m->vel[1] = approach_f32_symmetric(m->vel[1], ELEVATORSPEED, 8.f);
        } else if (SURFACETYPE(m->wall) == SURFACE_PAINTING_WOBBLE_AF) {
            m->vel[1] = approach_f32_symmetric(m->vel[1], 0.f, 8.f);
            // if mario is looking away from middle, sub 90 degrees, else add 90 degrees
            if (abs_angle_diff(atan2s(m->pos[2], m->pos[0]), m->faceAngle[1]) < 0x4000) {
                m->pos[0] += sins(atan2s(m->wall->normal.z, m->wall->normal.x) + 0x4000)
                             * ELEVATORSPEED /* * 2.f*/;
                m->pos[2] += coss(atan2s(m->wall->normal.z, m->wall->normal.x) + 0x4000)
                             * ELEVATORSPEED /* * 2.f*/;
            } else {
                m->pos[0] += sins(atan2s(m->wall->normal.z, m->wall->normal.x) - 0x4000)
                             * ELEVATORSPEED /* * 2.f*/;
...
extern Vec3f wkStart;
extern struct Surface *wkHit;
s32 act_air_hit_wall(struct MarioState *m) {
    vec3f_copy(wkStart, &m->pos[0]);
    wkHit = m->wall;
    if (m->heldObj != NULL) {
    ...
    if (m->input & ((INPUT_A_PRESSED | INPUT_A_LAST_FRAME))) {
        ...
        return set_mario_action(m, ACT_WALL_KICK_AIR, 0); // HERE
    } else {
        m->wallKickTimer = 5;
        if (m->vel[1] > 0.0f) {
            m->vel[1] = 0.0f;
...
s32 act_forward_rollout(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_NONE:
            if (m->actionState == 1) {
                if (m->marioObj->header.gfx.unk38.animFrame == 4) {
                    play_sound(SOUND_ACTION_SPIN, m->marioObj->header.gfx.cameraToObject);
                }
            } else {
                set_mario_animation(m, MARIO_ANIM_GENERAL_FALL);
    ...
    if (m->actionState == 1 && is_anim_past_end(m) && !justSet) {
        m->actionState = 2;
    }
    return FALSE;
}
...
s32 act_top_of_pole_jump(struct MarioState *m) {
    play_mario_jump_sound(m);
    common_air_action_step(m, ACT_FREEFALL_LAND, MARIO_ANIM_HANDSTAND_JUMP,
                           AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG);
    return FALSE;
}
...
s32 check_common_airborne_cancels(struct MarioState *m) {
    ...
    if (SURFACETYPE(m->floor) == SURFACE_VERTICAL_WIND
...
s32 mario_execute_airborne_action(struct MarioState *m) {
    ...
    u16 angle;
    s16 angleDiff;
#define FRAMESTODECEL 10.f
#define MAGFACTOR (64.f * FRAMESTODECEL)
    f32 magFactor;

    /*if (absf(m->inertia[2]) + absf(m->inertia[0])) {
        angle = atan2s(m->inertia[2], m->inertia[0]);
        angleDiff = angle - m->intendedYaw;
        magFactor = m->intendedMag / MAGFACTOR;
        m->inertia[0] = (((coss(angleDiff) + 1.f) / 2.f) * m->inertia[0] * (magFactor))
                        + m->inertia[0] * (1 - (magFactor));
        m->inertia[2] = (((coss(angleDiff) + 1.f) / 2.f) * m->inertia[2] * (magFactor))
                        + m->inertia[2] * (1 - (magFactor));
    }*/
