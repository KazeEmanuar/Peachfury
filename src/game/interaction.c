...
u32 determine_interaction(struct MarioState *m, struct Object *o) {
    ...
    if ((action & ACT_FLAG_ATTACKING)
        && ((gMarioState->action != ACT_GROUND_POUND) || (gMarioState->actionState > 0))) {
...
void mario_drop_held_object(struct MarioState *m) {
    if (m->heldObj != NULL) {
        ...
        m->heldObj->header.gfx.throwMatrix = 0;
...
u32 interact_coin(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    breath = approach_s16_symmetric(breath, MAXBREATH, 240 * o->oDamageOrCoinValue);
...
u32 interact_water_ring(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    breath = approach_s16_symmetric(breath, MAXBREATH, 40 * o->oDamageOrCoinValue);
    o->oInteractStatus = INT_STATUS_INTERACTED;
    return FALSE;
}
u8 isObjectYoshiStar(struct Object *a) {
    return (a->header.gfx.sharedChild == gLoadedGraphNodes[MODEL_YOSHI_STAR]);
}
extern s16 starIDHUD;
extern s16 whichMissionStar(u8 actNum);
extern void endMissionStar(u8 actNum);
u32 interact_star_or_key(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (m->health >= 0x100) {
        ...
        starIDHUD = starIndex;
        if (isObjectYoshiStar(o)) {
            starID = whichMissionStar(1 + ((o->oBehParams >> 24) & 0x1F));
            save_file_set_star_flags(gCurrSaveFileNum - 1, gLevelToCourseNumTable[starID / 8],
                                     (1 << (starID % 8)));
            endMissionStar(1 + ((o->oBehParams >> 24) & 0x1F));
            gLastCompletedStarNum = 0;
            gLastCompletedCourseNum = 77;
        } else {
...
u32 interact_warp(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (o->oInteractionSubtype & INT_SUBTYPE_FADING_WARP) {
        ...
        if (m->action != ACT_EMERGE_FROM_PIPE) {
            ...
            if (o->oBehParams & 0x04) {
                play_sound(SOUND_ACTION_SHRINK_INTO_BBH, gDefaultSoundArgs);
            } else {
            ...
            if (gCurrLevelNum == LEVEL_ENDING) {
                gShowFPS = (gMarioState->controller->buttonDown & Z_TRIG)
                           && (gMarioState->controller->buttonDown & R_TRIG);
                gCurrSaveFileNum = o->oBehParams >> 24;
                gMarioState->numStars = save_file_get_total_star_count(gCurrSaveFileNum - 1, 0, 0x18);
            }
            mario_stop_riding_object(m);
...
u32 interact_door(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (m->action == ACT_WALKING || m->action == ACT_DECELERATING) {
        ...
        if (actionArg & 0x00000001) {
            ...
            prevRoom = curRoom;
            curRoom = o->oBehParams & 0xff;
        } else {
            enterDoorAction = ACT_PUSHING_DOOR;
            prevRoom = curRoom;
            curRoom = (o->oBehParams >> 8) & 0xff;
        }
...
u32 interact_unknown_08(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (interaction & (INT_PUNCH | INT_KICK | INT_SLIDE_KICK)) {
...
u32 interact_damage(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    // CuckyDev: Make Bomb Mario immune to explosions
    if (o->behavior == segmented_to_virtual(bhvExplosion) && powerupSystem2 == 2)
        return FALSE;
    ...
    if (m->action != ACT_GETTING_BLOWN && capFlag != 0) {
        ...
        switch (capFlag) {
            case MARIO_VANISH_CAP:
                capTime = 840;
                capMusic = SEQUENCE_ARGS(4, SEQ_EVENT_ICECAP);
                goto playmusic;
                break;
...
u32 interact_grabbable(struct MarioState *m, u32 interactType, struct Object *o) {
    ...
    if (o->behavior == segmented_to_virtual(bhvKoopaShellUnderwater)) {
        m->interactObj = o;
        m->input |= INPUT_INTERACT_OBJ_GRABBABLE;
...
u32 mario_can_talk(struct MarioState *m, u32 arg) {
    ...
    if ((m->action == ACT_WALKING) || (m->action & ACT_FLAG_ALLOW_FIRST_PERSON)) {
        if (arg) {
            return TRUE;
        }
...
u32 interact_text(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    } else if (o->oInteractionSubtype & INT_SUBTYPE_CPTLOG) {
        interact = check_read_sign(m, o);
        push_mario_out_of_object(m, o, -10.0f);
    } else {
        push_mario_out_of_object(m, o, 2.0f);
...
void check_kick_or_punch_wall(struct MarioState *m) {
    ...
    if (m->flags & (MARIO_PUNCHING | MARIO_KICKING | MARIO_TRIPPING)) {
        ...
        if ((wallHit = resolve_and_return_wall_collisions(detector, 80.0f, 5.0f)) != NULL) {
            m->wallPunched = wallHit;
...
void check_lava_boost(struct MarioState *m) {
    if (!(m->action & ACT_FLAG_RIDING_SHELL) && m->pos[1] < m->floorHeight + 10.0f) {
        m->hurtCounter += (m->flags & MARIO_CAP_ON_HEAD) ? 8 : 8;
