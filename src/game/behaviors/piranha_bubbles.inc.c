...
void bhv_goomba_bubble_loop(void) {
    struct Object *parent = o->parentObj; // the Piranha Plant
    f32 scale = 0;
    s32 i;
    s32 frame = parent->header.gfx.unk38.animFrame * 1.25f;
    ...
    if (gCurrLevelNum == LEVEL_BOB) {
        cur_obj_set_pos_relative(parent, -20.f, 315.0f, 50.0f);
    } else if (gCurrLevelNum == LEVEL_BBH) {
        cur_obj_set_pos_relative(parent, -20.f, 20.0f, 65.0f);
    } else {
        cur_obj_set_pos_relative(parent, -50.f, 125.0f, 80.0f);
    }
        ...
        case PIRANHA_PLANT_BUBBLE_ACT_GROW_SHRINK_LOOP:
            ...
            if (parent->oAction == 0) {
                ...
                if (frame < doneShrinkingFrame) {
                    ...
                    scale =
                        sins(((frame - (lastFrame / 2.0f + 4.0f)) / beginGrowingFrame) * 0x4000) * 4.0f
                        + 1.0;
                } else {
                    scale = 1.0f;
                }
...
void bhv_yoshi_bubble_loop(void) {
    struct Object *parent = o->parentObj; // the Piranha Plant
    f32 scale = 0;
    s32 i;
    s32 frame = parent->header.gfx.unk38.animFrame * 1.25f;
    ...
    if (o->oBehParams2ndByte) {
        cur_obj_set_pos_relative(parent, -60.f, 48.0f, 45.0f);
    } else {
        cur_obj_set_pos_relative(parent, -78.f, 62.0f, 100.0f);
    }
    ...
    switch (o->oAction) {
        case 0:
            ...
            if (parent->oAction == 0) {
                ...
                if (frame < doneShrinkingFrame) {
                    scale = coss(frame / doneShrinkingFrame * 0x4000) * 2.0f + 1.0;
                } else if (frame > beginGrowingFrame) {
                    scale =
                        sins(((frame - (lastFrame / 2.0f + 2.0f)) / beginGrowingFrame) * 0x4000) * 2.0f
                        + 1.0;
                } else {
                    scale = 1.0f;
                }
