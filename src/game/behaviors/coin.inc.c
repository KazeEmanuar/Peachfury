...
extern void followPath(u32 *progress, s16 *trajectory, s16 rotSpeed, f32 range, f32 fVel);
void bhv_coin_formation_spawn_loop(void) {
    o->oAnimState++;
    o->oAnimState += !(random_u16() & 0x1F);
    if (o->oAnimState >= 16) {
        o->oAnimState -= 16;
    }
    if (o->oTimer == 0) {
    ...
    switch (o->parentObj->oBehParams >> 24) {
        case 1:
            o->oPosX = o->parentObj->oPosX
                       + sins(o->oBehParams2ndByte * 0x2000 + o->parentObj->oFaceAngleYaw) * 450.f;
            o->oPosZ = o->parentObj->oPosZ
                       + coss(o->oBehParams2ndByte * 0x2000 + o->parentObj->oFaceAngleYaw) * 450.f;
            break;
        case 2:
            if (o->oTimer > o->oBehParams2ndByte * 15) {
                followPath(&o->oOpacity, bbh_area_2_spline_CoinPath, 0x240, 500.f, 20.f);
                o->oPosY += sins(o->oTimer * 0x100) * 4.5f;
            } else {
                o->oPosX = o->parentObj->oPosX;
                o->oPosZ = o->parentObj->oPosZ;
            }
            break;
        case 3:
            if (o->oTimer > o->oBehParams2ndByte * 10) {
                followPath(&o->oOpacity, bbh_area_2_spline_CoinPathSnake, 0x400, 500.f, 20.f);
                o->oPosY += sins(o->oTimer * 0x100) * 4.5f;
            } else {
                o->oPosX = o->parentObj->oPosX;
                o->oPosZ = o->parentObj->oPosZ;
            ...
            break;
        case 4:
            if (o->oTimer > (o->oBehParams2ndByte+8) * 10) {
                followPath(&o->oOpacity, bbh_area_2_spline_CoinPathSnake, 0x400, 500.f, 20.f);
                o->oPosY += sins(o->oTimer * 0x100) * 4.5f;
            } else {
                o->oPosX = o->parentObj->oPosX;
...
void spawn_coin_in_formation(s32 sp50, s32 sp54) {
    ...
    switch (sp54 & 7) {
        ...
        case 5:
            sp38 = 0;
            sp40[0] = coss(sp50 << 13) * 175.0f;
            sp40[1] = sins(sp50 << 13) * 175.0f + 175.0f;
            if (sp50 > 3)
                sp3C = 0;
            break;
        case 6:
            ...
            break;
        case 7:
            sp40[0] = sins(sp50 << 13) * 450.0f;
            sp40[2] = coss(sp50 << 13) * 450.0f;
            break;
    }
    if (sp54 & 0x10)
        sp38 = 0;
    if (sp3C) {
        if (o->oBehParams & 0x00000001) {
            sp4C = spawn_object_relative(sp50, sp40[0], sp40[1], sp40[2], o, 0, bhvCoinFormationSpawn);
        } else {
...
extern u8 isEmu;
#define fakerenderdist (4000.f + isEmu * 45000.f)
#define /*0x10C*/ oCoinUnk10C OBJECT_FIELD_F32(0x21)
void bhv_coin_formation_init(void) {
    o->oCoinUnkF4 = (o->oBehParams >> 8) & 0xFF;
    o->oCoinUnk10C = fakerenderdist;
    if (gCurrLevelNum == LEVEL_CCM){
        o->oCoinUnk10C = fakerenderdist-750.f;
    }
}
void bhv_coin_formation_loop(void) {
    ...
    switch (o->oAction) {
        case 0:
            if (o->oDistanceToMario < o->oCoinUnk10C) {
        ...
        case 1:
            ...
            if (o->oDistanceToMario > (o->oCoinUnk10C + 100.f))
                o->oAction++;
            break;
        case 2:
            o->oAction = 0;
