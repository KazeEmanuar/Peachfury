 ...
 */
extern u8 isEmu;
#define FAKEDIST (isEmu * 100000.f)
void bhv_goomba_triplet_spawner_update(void) {
    ...
    if (o->oAction == GOOMBA_TRIPLET_SPAWNER_ACT_UNLOADED) {
        if (o->oDistanceToMario < 3000.0f + FAKEDIST) {
    ...
    } else if (o->oDistanceToMario > 4000.0f + FAKEDIST) {
        ...
        if ((o->oBehParams & 0x700) == 0x700) {
            obj_mark_for_deletion(o);
        }
}
extern u8 goomba_g002m00_rgba16[];
extern u8 goomba_g002m00closed_rgba16[];
extern u8 goombaeyeclosedblue[];
extern u8 goombaeyeopenblue[];
extern Gfx mat_goomba_g002m00_png_f3d[];
extern Gfx mat_goomba_sm64_material_006[];
/**
 * Initialization function for goomba.
 */
void bhv_goomba_init(void) {
    Gfx *a = segmented_to_virtual(mat_goomba_g002m00_png_f3d);
    ...
    o->oWallHitboxRadius = sGoombaHitbox.hurtboxRadius * sGoombaProperties[o->oGoombaSize].scale;
    ...
    if (o->oBehParams & 0xff) {
        // load goomba face texture new
        a[4].words.w1 = goombaeyeopenblue;
        a = segmented_to_virtual(mat_goomba_sm64_material_006);
        a[4].words.w1 = goombaeyeclosedblue;
        o->oDamageOrCoinValue = 2;
    } else {
        a = segmented_to_virtual(mat_goomba_g002m00_png_f3d);
        a[4].words.w1 = goomba_g002m00_rgba16;
        a = segmented_to_virtual(mat_goomba_sm64_material_006);
        a[4].words.w1 = goomba_g002m00closed_rgba16;
...
void goombaking_walk() {
    f32 animSpeed;
    if ((animSpeed = o->oForwardVel / 4.0f * 0.4f) < 1.0f) {
        animSpeed = 1.0f;
    }
    treat_far_home_as_mario(1200.0f);

    obj_forward_vel_approach(o->oGoombaRelativeSpeed * 2.0f, 2.f);

    if (o->oGoombaRelativeSpeed > 4.0f / 3.0f) {
        cur_obj_play_sound_at_anim_range(2, 17, SOUND_OBJ_GOOMBA_WALK);
    }

    if (cur_obj_lateral_dist_from_mario_to_home() < 1700.0f) {
        o->oGoombaTargetYaw = o->oAngleToMario;
        o->oGoombaRelativeSpeed = 10.0f;
    } else {
...
static void goomba_act_walk(void) {
    ...
    obj_forward_vel_approach(o->oGoombaRelativeSpeed * (o->oGoombaScale + (o->oBehParams & 0xff)),
                             0.4f);
    ...
    if (o->oGoombaTurningAwayFromWall) {
        ...
        cur_obj_rotate_yaw_toward(o->oGoombaTargetYaw, 0x200 + (o->oBehParams & 0xff) * 0x200);
    }
}
