...
void bobomb_act_patrol(void) {
    ...
    if ((obj_return_home_if_safe(o, o->oHomeX, o->oHomeY, o->oHomeZ, 400) == 1)
        && (object_facing_mario(gMarioState, o, 0x2000))) {
        o->oBobombFuseLit = 1;
        o->oAction = BOBOMB_ACT_CHASE_MARIO;
    }
    obj_check_floor_death(collisionFlags, o->oFloorPointer);
}
...
void bhv_bobomb_buddy_init(void) {
    ...
    if (gCurrLevelNum == LEVEL_CASTLE_GROUNDS) {
        ...
        if (save_file_get_total_star_count(gCurrSaveFileNum - 1, 0, 0x18) >= 15) {
            o->oBehParams2ndByte = 0x3D;
            o->oBehParams = 0x003D0000;
        }
    }
}
...
void bobomb_buddy_act_turn_to_talk(void) {
    ...
    //if ((sp1e == 5) || (sp1e == 16)) //CuckyDev: 'if' was falling through to the turn line
    //    cur_obj_play_sound_2(SOUND_OBJ_BOBOMB_WALK);
