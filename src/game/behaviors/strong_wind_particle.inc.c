...
void bhv_dino_nose_particle(void) {
    struct Object *penguinObj;
    f32 distanceFromPenguin;
    f32 penguinXDist, penguinZDist;
    ...
    if (o->oTimer == 0) {
        obj_translate_xyz_random(o, 5.0f*o->oBehParams2ndByte);//spread
        
        o->oForwardVel = coss(o->oMoveAnglePitch) * 40.0f;
        o->oVelY = sins(o->oMoveAnglePitch) * -40.0f;
        o->oOpacity = 100;
    }
    
    cur_obj_move_using_fvel_and_gravity();
    if (o->oTimer > 10) // Deactivate after 15 frames
        obj_mark_for_deletion(o);
