...
f32 transitionLerpFade[4] = { 0 };
f32 transitionLerpTexture[2] = { 0 };
...
s32 set_and_reset_transition_fade_timer(s8 fadeTimer, u8 transTime) {
    if (sTransitionColorFadeCount[fadeTimer] == transTime) {
        ...
        transitionLerpFade[fadeTimer] = 0;
        transitionLerpTexture[fadeTimer] = 0;
...
u8 set_transition_color_fade_alpha(s8 fadeType, s8 fadeTimer, u8 transTime) {
    ...
    switch (fadeType) {
        case 0:
            time = (f32) transitionLerpFade[fadeTimer] * 255.0 / (f32) (transTime - 1) + 0.5; // fade in
            break;
        case 1:
            time =
                (1.0 - transitionLerpFade[fadeTimer] / (f32) (transTime - 1)) * 255.0 + 0.5; // fade out
            break;
    }
    return time;
}
...
s16 calc_tex_transition_radius(s8 fadeTimer, s8 transTime, struct WarpTransitionData *transData) {
    ...
    f32 radiusTime = transitionLerpFade[fadeTimer] * texRadius / (f32) (transTime - 1);
...
f32 calc_tex_transition_time(s8 fadeTimer, s8 transTime, struct WarpTransitionData *transData) {
    ...
    f32 result = (f32) transitionLerpFade[fadeTimer] * sqrtfXY / (f32) (transTime - 1);
...
void make_tex_transition_vertex(Vtx *verts, s32 n, s8 fadeTimer, struct WarpTransitionData *transData,
    ...
    u8 g = transData->green;
    u8 b = transData->blue;
    u16 zeroTimer = transitionLerpTexture[fadeTimer];
...
int render_screen_transition(s8 fadeTimer, s8 transType, u8 transTime,
    ...
    if (sTransitionColorFadeCount[fadeTimer] != 0) {
        if (!gMoveSpeed) {
            transitionLerpFade[fadeTimer] = sTransitionColorFadeCount[fadeTimer];
        } else {
            if (gMoveSpeed == 1) {
                transitionLerpFade[fadeTimer] =
                    approach_pos(transitionLerpFade[fadeTimer], sTransitionColorFadeCount[fadeTimer]);
            } else {
                transitionLerpFade[fadeTimer] =
                    approach_posF(transitionLerpFade[fadeTimer], sTransitionColorFadeCount[fadeTimer]);
            }
        }
    }
    if (sTransitionTextureFadeCount[fadeTimer] != 0) {
        if (!gMoveSpeed) {
            transitionLerpTexture[fadeTimer] = sTransitionTextureFadeCount[fadeTimer];
        } else {
            if (gMoveSpeed == 1) {
                transitionLerpTexture[fadeTimer] = approach_pos(transitionLerpTexture[fadeTimer],
                                                                sTransitionTextureFadeCount[fadeTimer]);
            } else {
                transitionLerpTexture[fadeTimer] = approach_posF(
                    transitionLerpTexture[fadeTimer], sTransitionTextureFadeCount[fadeTimer]);
            }
        }
    }
...
Gfx *render_cannon_circle_base(void) {
    ...
    gDPLoadTextureBlock(g++, sTextureTransitionID[TEX_TRANS_CIRCLE], G_IM_FMT_IA, G_IM_SIZ_8b, 32, 64,
                        0, G_TX_WRAP | G_TX_MIRROR, G_TX_WRAP | G_TX_MIRROR, 5, 6, G_TX_NOLOD,
                        G_TX_NOLOD);
    ...
    gSPBranchList(g++, dl_screen_transition_end);
    //gSPEndDisplayList(g);
    return dlist;
}
...
s32 transition_logic(s8 fadeTimer, s8 transType, u8 transTime, struct WarpTransitionData *transData) {
