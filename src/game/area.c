...
struct ObjectWarpNode *area_get_warp_node_from_params(struct Object *obj) {
    u8 sp1F = (obj->oBehParams & 0x00FF0000) >> 16;
...
void screen_transition_logic(void) {
    ...
    if (gWarpTransition.isActive) {
        if (gWarpTransDelay == 0) {
            gWarpTransition.isActive =
                !transition_logic(0, gWarpTransition.type, gWarpTransition.time, &gWarpTransition.data);
            if (!gWarpTransition.isActive) {
...
s16 SCREEN_WIDTH_MATH = 320;
s16 SCREEN_HEIGHT_MATH = 240;
s16 BORDER_HEIGHT_MATH = 0;
s16 BORDER_WIDTH_MATH = 0;
void render_game(void) {
    if (gCurrentArea != NULL && !gWarpTransition.pauseRendering) {
        if (viewportClipSize > 0) {
            sEndCutsceneVp.vp.vscale[0] = 640;
            sEndCutsceneVp.vp.vscale[1] = 480 - (viewportClipSize & 0xFFFC);
        ...
        gDPSetScissor(gDisplayListHead++, G_SC_NON_INTERLACE, 0, 0, SCREEN_WIDTH,
                      SCREEN_HEIGHT); // trust me, you need this (and i dont know why)
                render_hud();
        render_text_labels();
