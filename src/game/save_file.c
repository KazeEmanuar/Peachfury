...
s32 osEepromLongDirty(OSMesgQueue *mq, u8 address, u8 *buffer, int nbytes) {
    s32 status = 0;
...
static s32 read_eeprom_data_dirty(void *buffer, s32 size) {
    s32 status = 0;
    ...
    if (gEepromProbe != 0) {
        ...
        do {
            triesLeft--;
            status = osEepromLongDirty(&gSIEventMesgQueue, offset, buffer, size);
        } while (triesLeft > 0 && status != 0);
    }
    return status;
}
...
static s32 write_eeprom_data_dirty(void *buffer, s32 size) {
    s32 status = 1;
    ...
    if (gEepromProbe != 0) {
        ...
        do {
            triesLeft--;
            status = osEepromLongWriteDirty(&gSIEventMesgQueue, offset, buffer, size);
        } while (triesLeft > 0 && status != 0);
    }
...
void save_file_collect_star_or_key(s16 coinScore, s16 starIndex) {
    ...
    s32 starFlag;
    courseIndex+=starIndex/4;
    starFlag = 1 << (starIndex&3);
...
void save_file_set_statetimer() {
    gSaveBuffer.files[gCurrSaveFileNum - 1][0].savestateTimer++;
    gSaveBuffer.files[gCurrSaveFileNum - 1][1].savestateTimer =
        gSaveBuffer.files[gCurrSaveFileNum - 1][0].savestateTimer;
    write_eeprom_data_dirty(&gSaveBuffer.files[gCurrSaveFileNum - 1]->savestateTimer, 8);
}

u32 save_file_get_statetimer() {
    struct SaveFile *saveFile = &gSaveBuffer.files[gCurrSaveFileNum - 1][0];
    return saveFile->savestateTimer;
}

u32 save_file_get_statetimer2() {
    read_eeprom_data_dirty(&gSaveBuffer.files[gCurrSaveFileNum - 1]->savestateTimer, 8);
    return gSaveBuffer.files[gCurrSaveFileNum - 1]->savestateTimer;
}
