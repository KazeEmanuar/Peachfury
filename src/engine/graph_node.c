...
struct GraphNodeScale *init_graph_node_scale(struct AllocOnlyPool *pool,
    ...
    if (graphNode != NULL) {
        ...
        graphNode->node.flags = (drawingLayer << 8) | (graphNode->node.flags & 0xFF);
        graphNode->scale = scale;
        graphNode->deltaCurrentScale = 0.01f;
        graphNode->displayList = displayList;
    }
...
struct GraphNodeObject *init_graph_node_object(struct AllocOnlyPool *pool,
    ...
    if (graphNode != NULL) {
        ...
        vec3f_copy(graphNode->deltaCurrentPos, pos);
        vec3f_copy(graphNode->deltaCurrentScale, scale);
        vec3s_copy(graphNode->deltaCurrentRot, angle);
 ...
 */
inline s32 retrieve_animation_index(s32 frame, u16 **attributes) {
    register s32 result;
 ...
 * the end etc.
 */
s32 geo_update_animation_frame(struct GraphNodeObject_sub *obj, s32 *accelAssist) {
    s32 result;
