...
extern u32 last10Timers[4][10];
extern u8 isEmu;


extern u8 isEverdrive;
u8 isEmulator() {
    volatile u32 *romProbe = 0xB0000000;
    *romProbe = 0x11223344;
    if (!isEverdrive) {
        return isEmu = 1;
    }
    if (IO_READ(DPC_PIPEBUSY_REG) + IO_READ(DPC_PIPEBUSY_REG) + IO_READ(DPC_PIPEBUSY_REG) == 0) {
        return isEmu = 1;
    } else {
        return isEmu = 0;
    }
...
void geo_layout_cmd_node_root(void) {
    ...
    if (isEmu) {
        osViModeTable[2].comRegs.vSync = 2100;

        SCREEN_WIDTH_MATH = 320;
        SCREEN_HEIGHT_MATH = 240;
        BORDER_HEIGHT_MATH = 0;
        BORDER_WIDTH_MATH = 0;
        unlimitFPS = 1;
        wisdeScreenMode = 1;
    } else {
        SCREEN_WIDTH_MATH = 319 - 8;  // 319-8??????????
        SCREEN_HEIGHT_MATH = 239 - 8; // 239-8??????????
        BORDER_HEIGHT_MATH = 8;
        BORDER_WIDTH_MATH = 8;
        unlimitFPS = 0; // supposed to be off, but that adds a shit ton of lag for some reason
        wisdeScreenMode = 0;
    }
    width = (SCREEN_WIDTH_MATH - BORDER_WIDTH_MATH) / 2;
    height = (SCREEN_HEIGHT_MATH - BORDER_HEIGHT_MATH) / 2;
...
void geo_layout_cmd_node_culling_radius(void) {
    ...
    if (isEmu) {
        graphNode =
            init_graph_node_culling_radius(gGraphNodePool, NULL, MIN(32500, cur_geo_cmd_s16(0x02) * 2));
    } else {
