...
static s32 bhv_cmd_call_segmented(void) {
    NativeBhvFunc behaviorFunc = ((u32) segmented_to_virtual(BHV_CMD_GET_VPTR(1))) | 0x80000000;
...
void cur_obj_update(void) {
    s32 objFlags;
    f32 distanceFromMario; // NECESSARY
    BhvCommandProc bhvCmdProc;
    s32 bhvProcResult;
    if (gCurrentObject->oRoom) {
        if ((((gCurrentObject->oRoom) & 0xFF) == curRoom)
            || (((gCurrentObject->oRoom >> 8) & 0xFF) == curRoom)
            || (((gCurrentObject->oRoom >> 16) & 0xFF) == curRoom)
            || (gCurrentObject->oRoom >> 24 == curRoom)) {
            cur_obj_enable_rendering();
            gCurrentObject->activeFlags &= ~ACTIVE_FLAG_IN_DIFFERENT_ROOM;
        } else if ((gMarioState->action == ACT_PULLING_DOOR)
                   || (gMarioState->action == ACT_PUSHING_DOOR)) {
            if ((((gCurrentObject->oRoom) & 0xFF) == prevRoom)
                || (((gCurrentObject->oRoom >> 8) & 0xFF) == prevRoom)
                || (((gCurrentObject->oRoom >> 16) & 0xFF) == prevRoom)
                || (gCurrentObject->oRoom >> 24 == prevRoom)) {
                cur_obj_enable_rendering();
                gCurrentObject->activeFlags &= ~ACTIVE_FLAG_IN_DIFFERENT_ROOM;
            }
        } else {
    ...
    if (currentScene) {
        f32 x, y, z;
        x = gCurrentObject->oPosX - gLakituState.pos[0];
        y = gCurrentObject->oPosY - gLakituState.pos[1];
        z = gCurrentObject->oPosZ - gLakituState.pos[2];
        gCurrentObject->oDistanceToMario = sqrtf(x * x + y * y + z * z);
    ...
    /*if (objFlags & OBJ_FLAG_SET_FACE_ANGLE_TO_MOVE_ANGLE) {
        gCurrentObject->oFaceAnglePitch = gCurrentObject->oMoveAnglePitch;
        gCurrentObject->oFaceAngleYaw = gCurrentObject->oMoveAngleYaw;
        gCurrentObject->oFaceAngleRoll = gCurrentObject->oMoveAngleRoll;
    }*/

    if (objFlags & OBJ_FLAG_SET_FACE_YAW_TO_MOVE_YAW) {
        gCurrentObject->oFaceAngleYaw = gCurrentObject->oMoveAngleYaw;
    }
    if (objFlags & OBJ_FLAG_GET_FLOOR_HEIGHT){
    ...
    if (objFlags & OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE) {
        ...
        gCurrentObject->header.gfx.angle[0] = gCurrentObject->oFaceAnglePitch;
        gCurrentObject->header.gfx.angle[1] = gCurrentObject->oFaceAngleYaw;
        gCurrentObject->header.gfx.angle[2] = gCurrentObject->oFaceAngleRoll;
    }

    if (gCurrentObject->header.gfx.unk38.curAnim != NULL) {
        gCurrentObject->header.gfx.unk38.animFrame = geo_update_animation_frame(
            &gCurrentObject->header.gfx.unk38, &gCurrentObject->header.gfx.unk38.animFrameAccelAssist);
    }
    ...
    /*if (gCurrentObject->oRoom != -1) {//og sm64 way of doing rooms
        cur_obj_enable_rendering_if_mario_in_room();
    } else */
    if (isEmu) {
        gCurrentObject->oDrawingDistance = 200000.f;
    }
    if (!(objFlags & OBJ_FLAG_ACTIVE_FROM_AFAR)) {
        ...
        if ((distanceFromMario > gCurrentObject->oDrawingDistance)) {
            ...
            if (!currentScene) {
                gCurrentObject->header.gfx.firstBit = 0;
                gCurrentObject->header.gfx.node.flags &= ~GRAPH_RENDER_ACTIVE;
